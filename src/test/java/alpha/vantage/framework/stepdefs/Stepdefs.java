package alpha.vantage.framework.stepdefs;
...
import alpha.vantage.framework.base.AbstractSteps;
...
public class Stepdefs extends AbstractSteps {
    ...
    public void userGetResponseWithExchangeRateForCurrentDate(DataTable dataTable) {
        ...
        CurrencyExchangeDTO actualResponseData = getCurrencyExchangeDataFromJsonString(
            testContext().getResponse().getBody().asString());
    ...
    public void userGetResponseWithGlobalQuoteDataForWithCorrectDataFormat(String expectedSymbol) {
        GlobalQuoteDTO globalQuoteDTO = getGlobalQuoteDataFromJsonString(
            testContext().getResponse().getBody().asString());
    ...
    public void userGetResponseInCsvFormat() {
        testContext().getResponse().then().assertThat()
                     .header(CONTENT_DISPOSITION, containsString(EXPECTED_GLOBAL_QUOTE_CSV_FILE));
